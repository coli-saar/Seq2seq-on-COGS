...
@DatasetReader.register("cogs_qa_reader")
    ...
    def _read(self, file_path: str) -> Iterator[Instance]:
        ...
        with open(cached_path(file_path), encoding='utf-8', mode='r') as infile:
            for line_num, row in enumerate(csv.reader(infile, delimiter="\t")):
                ...
                if len(row) == 4:
                    sentence, question, ans, gen_type_required = row
                elif len(row) == 5:
                    sentence, question, ans, gen_type_required, _ = row
                ...
                yield self.text_to_instance(source_tokens=source_tokens+quest_tokens,
                                            target_tokens=ans_tokens,
