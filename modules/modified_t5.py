...
from modules.model.transformers.T5 import fastT5

@Model.register("cogs_t5")
class T5(Model):
    def __init__(
        self,
        vocab: Vocabulary,
        model_name: str,
        beam_search: Lazy[BeamSearch] = Lazy(BeamSearch, beam_size=4, max_steps=100),
    ...
    def forward(  # type: ignore
        ...
        if self.training:
        ...
        elif 'test_flag' in kwargs and not kwargs['test_flag']:
            assert output.loss is not None
            output_dict["loss"] = output.loss
        else:
            ...
            readable_output_dict = self.make_output_human_readable(output_dict)
            return readable_output_dict
