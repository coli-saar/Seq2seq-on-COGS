import warnings
from typing import Dict, Tuple, Any, cast, Union, List
import numpy
...
class Bart(Model):
    ...
    def forward(
        ...
        if self.training:
            ...
            # print("##########output loss ##########")
            # print(outputs["decoder_logits"].size())
            # print(outputs["decoder_logits"].detach().cpu().numpy()[0, :, :10])
            # print(cast(torch.LongTensor, target_ids[:, 1:].contiguous()).detach().cpu().numpy())
            # print(outputs["loss"].detach().cpu().numpy())
    ...
    def make_output_human_readable(self, output_dict: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        predicted_texts = self._indexer._tokenizer.batch_decode(
            predictions.tolist(), skip_special_tokens=True
        )
        output_dict["predicted_text"] = []
        for predicted_text in predicted_texts:
            predicted_text = predicted_text.lstrip().replace('.', ' . ').replace(',', ' , ')
            predicted_text = ' '.join(predicted_text.split())
            output_dict["predicted_text"].append(predicted_text)
        return output_dict
