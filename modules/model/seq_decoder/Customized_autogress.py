...
from allennlp.nn.beam_search import BeamSearch, Sampler, MultinomialSampler
...
@SeqDecoder.register("customized_autogressive_seq_decoder")
class CustomizedAutoRegressiveSeqDecoder(SeqDecoder):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "multinomial_sampler" in kwargs:
            # beam_search_extras["sampler"] = kwargs["sampler"]
            # warnings.warn(deprecation_warning.format("sampler"), DeprecationWarning)
            sampler = MultinomialSampler()
        else:
            sampler = None
        self._beam_search = beam_search.construct(
            end_index=self._end_index, vocab=self._vocab, sampler=sampler, **beam_search_extras
        )
        ...
        # self._combine_layer = Linear(self.target_embedder.weight.shape[-1]+self._decoder_net.get_output_dim() * 2, \
        ...
        # self.dropout_out = torch.nn.Dropout(p=dropout)
    ...
    def _forward_loss(
        ...
        if self._scheduled_sampling_ratio == 0 and self._decoder_net.decodes_parallel:
            _, decoder_output, output_features = self._decoder_net(
    ...
    def _prepare_output_projections(
        ...
        if previous_steps_predictions is None or previous_steps_predictions.shape[-1] == 0:
            ...
            # last_feature = last_predictions_embeddings.new_zeros()
        else:
            # shape: (group_size, steps_count, target_embedding_dim)
            ...
            # last_feature = last_output_features

        decoder_output_features = state.get("decoder_output_features")

        if decoder_output_features is None or previous_steps_predictions.shape[-1] == 0:
            # shape: (group_size, 1, target_embedding_dim)
            decoder_output_features = last_predictions_embeddings.new_zeros(last_predictions_embeddings.shape).squeeze(1)

        decoder_state, decoder_output, output_features = self._decoder_net(
            previous_state=state,
            encoder_outputs=encoder_outputs,
            source_mask=source_mask,
        ...
        output_projections = self._output_projection_layer(output_features)
